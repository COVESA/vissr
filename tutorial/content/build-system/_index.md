...
To test the "VISSR tech stack", the server together with data store and feeder,
for testing of the VehicleData profile the runtest.sh is available in the root directory.
For testing of the Service profile the runtest-service.sh is available in the root directory.
These bash scripts start the server and the feedervx (x is the latest of existing feeder templates) in the background,
and then the testClient is started in the terminal window.
The testClient reads the testRequests-vehicleData.json (for VehicleData profile),
 or the testRequests-service.json (for Service profile) file for the requests it will issue to the different transport protocols,
...
allowing the operator to get time to inspect the request/response logs before continuing.
...
This number is set in runTest.sh by the CLI parameter "-m 4". Other numbers of events can be set by editing this number.

Issue the following command in the root directory to run the tests:
```
For VehicleData profile:
$ ./runtest.sh startme
For Service profile:
$ ./runtest-service.sh startme
```
When the testing is completed the script should be run to terminate the server and feeder processes:
```
$ ./runtest.sh stopme
or
$ ./runtest-service.sh stopme
```
The files testRequests-vehicleData.json/testRequests-service.json/ can be edited to add or remove test cases.
It is up to the operator to ensure that subscribe trigger conditions will be met by the simulated vehicledata that the feeder injects.
This data can be modified in the tripdata.json file in the feedervx directory.

The simulated service data is generated by the service implementations.
...
there is the runstack.sh script that builds and starts the server and the feedervx, with the feeder generating simulated input.
This can be used for both VehicleData and Service profile testing.
